...
/* Distortions */
half4 pixelate(sampler2D tex, half4 _MainTex_ST, half2 uv, half scale, half ratio)
{
	half ds = 1.0 / scale;
	half2 coord = half2(ds * ceil(uv.x / ds), (ds * ratio) * ceil(uv.y / ds / ratio));
	return half4(tex2D(tex,UnityStereoScreenSpaceUVAdjust(coord, _MainTex_ST)).xyzw);
}
