...
Shader "Hidden/Colorful/Chromatic Aberration"
{
	...
	CGINCLUDE
		...
		half3 compute(half2 uv)
		{
			...
			return half3(tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(uv_r, _MainTex_ST)).r, tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(uv_g, _MainTex_ST)).g, tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(uv_b, _MainTex_ST)).b);
		}
